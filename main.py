...
if __name__ == '__main__':
    # res = minimize(to_minimize, np.array(np.random.randn(3)), method='BFGS', options={'disp': True})
    # res = minimize(to_minimize, np.array(np.random.randn(3)), method='L-BFGS-B', options={'disp': True})
    # print(res.x)
    # kp, ki, kd = res.x[0], res.x[1], res.x[2]
    history, measurement_history, times, kalman, _, _ = simulate(1, 0.1, 0)
